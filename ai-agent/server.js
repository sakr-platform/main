...
app.post('/api/telegram-content-notification', async (req, res) => {
  ...
  try {
    ...
    if (!telegramBotToken || !groupId || !name) return res.status(400).json({ error: 'Notification details are required' });
    const subscriptions = await getTelegramSubscriptions();
    const recipients = subscriptions.filter((subscription) => subscription.groupId === String(groupId));
    let sent = 0;
